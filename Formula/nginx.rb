class Nginx < Formula
  ...
  bottle do
    ...
    sha256 "5c4a0c54f1bfd5685e5d9272edfc50e8f709e02d80878f0b8dbdd6444960bfb4" => :high_sierra
  end
  ...
  resource "headers-more-nginx-module" do
    url "https://github.com/openresty/headers-more-nginx-module.git",
        :tag => "v0.33"
  end

  resource "nginx_mod_h264_streaming" do
  ...
  resource "nginx-push-stream-module" do
    url "https://github.com/wandenberg/nginx-push-stream-module.git",
        :tag => "0.5.4"
  end

  resource "ngx_devel_kit" do
    url "https://github.com/simpl/ngx_devel_kit.git",
        :tag => "v0.3.0"
  end

  resource "lua-nginx-module" do
    url "https://github.com/openresty/lua-nginx-module.git",
        :tag => "v0.10.13"
  end

  resource "echo-nginx-module" do
    url "https://github.com/openresty/echo-nginx-module.git",
        :tag => "v0.61"
  end

  def install
    resource("headers-more-nginx-module").stage do |staging|
      @headers_more_nginx_module = Dir.pwd
      staging.retain!
    end

    resource("nginx_mod_h264_streaming").stage do |staging|
      @nginx_mod_h264_streaming = Dir.pwd
      p = Patch.create(:p2, :DATA)
      p.path = Pathname.new(__FILE__).expand_path
      p.apply
      staging.retain!
    end

    resource("nginx-push-stream-module").stage do |staging|
      @nginx_push_stream_module = Dir.pwd
      staging.retain!
    end

    resource("ngx_devel_kit").stage do |staging|
      @ngx_devel_kit = Dir.pwd
      staging.retain!
    end

    resource("lua-nginx-module").stage do |staging|
      @lua_nginx_module = Dir.pwd
      staging.retain!
    end

    resource("echo-nginx-module").stage do |staging|
      @echo_nginx_module = Dir.pwd
      staging.retain!
    end
    ...
    args = %W[
      ...
      --add-module=#{@nginx_push_stream_module}
